...
function barebones:OnNPCSpawned(keys)
	...
	-- split golems seem to spawn inside eachother, causing them to bug out. 
	print("[debug] npc named: " .. npc:GetUnitName() .. " has spawned.")
	if npc:GetUnitName() == "npc_dota_neutral_mud_golem_split" then
		FindClearSpaceForUnit( npc, npc:GetAbsOrigin(), true )
	else
	end
...
function barebones:OnHeroInGame(hero)
	...
	gamemode.player_hero = hero
	hero:AddAbility( "sa_hero_bonuses" )
	local abil_z = hero:FindAbilityByName("sa_hero_bonuses")
	abil_z:SetLevel( 1 )
	if IsInToolsMode() then
	else
		abil_z:SetHidden( true )
	end
end
